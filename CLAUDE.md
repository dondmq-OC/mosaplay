...
### 评分优先于规则
当需要在多个方案中做选择时：列候选集 → 设计可量化的评分函数 → 遍历选最优。不要写 if-else 启发式链。评分函数天然可解释、可维护、可改进（MosaPlay 布局算法的 3 次迭代证明了这一点：固定宫格 → flexbox → 暴力搜索+评分，最简洁的是最后者）。

### 平台约束优先于代码逻辑
跨平台 bug 的根因通常是平台本身的约束，而非代码错误。查 Linux/macOS/Windows 各平台 API 规格书应早于查代码。macOS Metal 上 OpenGL Core Profile 限制 glLineWidth=1.0，这不是 bug 而是规格——花了几轮迭代才意识到。

### 自动化构建链优于手动跨平台
当跨平台构建复杂时，把过程编码为 CI pipeline，而非在本地手动折腾交叉编译。6 轮 CI 调试后获得了一个可重现的构建，macOS 交叉编译至今没成功过。

### 用户路径与开发者路径不同
双击 .app、拖放文件、无参数启动——这些 GUI 操作路径的错误处理与命令行调用完全不同。测试必须在实际部署场景下进行。本项目中"闪退"和"拖放不生效"两个 bug 都是因为只在终端中测试。

### 遍历+评分胜于启发式
布局算法的演进：固定宫格 → 比例 flexbox → 暴力搜索所有网格+分栏。最复杂的启发式规则最终被最朴素的方法击败。遇到多约束问题时，先问"能不能枚举候选然后评分"，而不是"能不能写规则来约束"。

### 并行对比胜于序贯判断
图标设计 16 版迭代后回到最初——因为一直在序贯生成、提交、等待反馈。后期改用 4×4 对比图，一次决策 16 个方案。任何视觉/UX/设计方案决策，都应先生成并置对比图再评审。

### 验证链：编译 → 冒烟 → CI → 用户实测
每一级验证发现不同维度的 bug。编译检查类型错误，冒烟检查运行时崩溃，CI 检查跨平台集成，用户实测发现交互设计缺陷。跳过任何一级都会导致后续迭代浪费。本项目中 SIGSEGV（缺哨兵）被冒烟捕获，布局黑边被用户实测发现。
